...
本文件是 `parallel-agent-dispatch` 的配套参考，定义"合入主分支并最终回收"阶段的完整流程。只有用户在当前任务中明确命令把父任务分支合入主分支时才执行本阶段。继续使用任务开始时确认并记录的 `mainBranch`、父任务分支和 `task-slug`；任一身份无法精确回读时停止询问，不得按名称猜测。
