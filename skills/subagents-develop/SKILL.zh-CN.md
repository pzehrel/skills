...
7. （仅用户再次显式命令时）合入主分支并最终回收，见 `references/merge-to-main.zh-CN.md`。
...
用户在本次任务中之后明确下令把父任务分支合入主分支时，才授权进入"合入主分支并最终回收"阶段（流程见 `references/merge-to-main.zh-CN.md`）。该命令仍不授权 push、删除远端分支、强制删除或丢弃内容。
...
立即按 `references/safety-inventory.zh-CN.md` 的"创建回读"规程回读 branch、HEAD、tree 和干净状态，全部匹配才继续。父任务分支创建后，即使主分支被推进，也不得自动 rebase 或移动 base；最终汇报原始 base 和当前差异。
...
逐个创建并按 `references/safety-inventory.zh-CN.md` 回读确认后再派发。并发 subagent 数量不得超过当前可用 agent slots。
...
只有子任务已合入父任务分支、父分支组合检查没有要求回退该成果、子 worktree 干净且 subagent 已结束时，才执行清理。清理的安全盘点与删除规程见 `references/safety-inventory.zh-CN.md`，核心规则：
...
**清理是完成定义的一部分，不是可选项。** 每个波次成功后立即清理该波次子资源。进入移交阶段前，必须执行"子资源清零验证"（枚举命令见 `references/safety-inventory.zh-CN.md`）：当前 `task-slug` 前缀下除 `integration` 外不得残留任何本地分支、worktree 或 worktree 根下的磁盘项；验证输出必须纳入最终报告。清零验证未通过时不得声称开发完成，继续清理或明确报告保留原因。
...
完整的查漏、候选生成、主分支 worktree 选择和回收顺序见 `references/merge-to-main.zh-CN.md`，必须严格按该文档执行。
